...
import { currentUser, toggleFollowUser } from '../store/db.js';
...
export function setupProfile() {
    ...
    // ==========================================
    // 2. VIEWING OTHERS & FOLLOW SYSTEM
    // ==========================================
    // Listens for clicks on user names in the posts feed
    document.addEventListener('click', async (e) => {
        ...
        if (profileBtn) {
            ...
            try {
                ...
                // Render Enhanced Profile Header
                        ...
                        <div class="flex-grow w-full">
                            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
                                <div>
                                    ...
                                    <div class="flex gap-4 text-sm text-slate-300 font-medium">
                                        <span class="hover:text-white transition cursor-pointer"><strong>${followersCount}</strong> Followers</span>
                                        <span class="hover:text-white transition cursor-pointer"><strong>${followingCount}</strong> Following</span>
                                    </div>
                                ...
                                <div class="flex flex-col gap-2 min-w-[140px]">
                                    ${!isMe ? `
                                        <button id="profile-follow-btn" data-target-email="${targetEmail}" class="w-full py-2 px-4 rounded-full font-bold text-sm transition-all ${isFollowing ? 'bg-slate-800 text-white hover:bg-slate-700 border border-slate-700' : 'bg-sky-500 text-white hover:bg-sky-600 shadow-lg shadow-sky-500/20'}">
    ...
    // ==========================================
    // 3. FOLLOW BUTTON LISTENER
    // ==========================================
    document.addEventListener('click', async (e) => {
        if (e.target.id === 'profile-follow-btn') {
            ...
            try {
                const isNowFollowing = await toggleFollowUser(targetEmail);
                if (isNowFollowing) {
                    btn.textContent = 'Following';
                    btn.className = 'w-full py-2 px-4 rounded-full font-bold text-sm transition-all bg-slate-800 text-white hover:bg-slate-700 border border-slate-700';
                } else {
                    btn.textContent = 'Follow';
                    btn.className = 'w-full py-2 px-4 rounded-full font-bold text-sm transition-all bg-sky-500 text-white hover:bg-sky-600 shadow-lg shadow-sky-500/20';
                }
            } catch (err) {
